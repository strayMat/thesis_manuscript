...
\begin{background_box_left}
  ...
  \textcolor{h_color}{For example, only 6\% of asthmatics would have been eligible for their own
...
\begin{chapterabstract}\label{sec:predictive_models:abstract}%
  ...
  generally present a strong class imbalance, which complicates models
  development and undermines their performances. Further work is needed to
  understand which types of models well in these scenarios where the number of
  cases requires good sample efficiency.
\end{chapterabstract}
...
\subsection{Low prevalence and local practice call for data frugality}%
...
Figure \ref{fig:pipelines:count_encoding}. To better take into account the
temporal dimension, we also computed a decayed count of the events in the
patient history, noted $C_{\delta}$. For an event triplet $(i, t, c)$, we
compute the time delta between the event and the followup time.
$$\Delta t = |t-T_0|$$
...
\begin{figure}
  \includegraphics[width=0.7\textwidth]{img/chapter_3/patient_counter_with_decay.pdf}
...
some sense of clinical relation. Building upon this algorithm, we propose two
...
\paragraph{Decayed counting followed by random forest outperforms elaborated
  embedding models}%
