...
\begin{chapterabstract}[Abstract]
  ...
  simulation and three semi-simulated datasets, we shows that the usual machine
  learning risk are not adapted to the causal setting and that the doubly robust
...
\begin{chapterabstract}[Résumé en français]
  ...
  semi-simulées, nous montrons que le risque usuel en apprentissage statistique
  n'est pas adapté au cadre causal et que le risque R doublement robuste surpasse
...
them --a processed called pseudonymization \citep{dernoncourt2017identification,
  paris2019desidentification}. For engineers and NLP researchers, this routine
care data are different from traditional research data collection because it
requires innovating tools --eg. from NLP-- to deal with the novel complexity and
scale of the data collection process. For epidemiologists and physicians, the
major difference lies in the opposition between experimental and observational
data (presented in Subsection
...
%\md{develop: schneeweiss and the replicate progra?m}.
% % too much
...
\begin{chapterabstract}\label{sec:predictive_models:abstract}%
  ...
  predictive algorithms. Text and image put apart, this data can be represented
  as time-stamped medical codes with a high number of categories. Current
  state-of-the-art predictive models for EHRs build on increasingly elaborated
  pipelines --based on the transformer architecture-- to handle the complexity
  of these data. Acknowledging the difficulty to deploy, transfer and adapt
  these models on local care environments, we explore a complexity-benefit
  ...
  clinically relevant problems. We introduce a simple aggregation of static
  embeddings --transferred from national claims and publicly available--,
  ...
  their performances. Further work is needed to understand which types of models
  well in these scenarios where the number of cases requires good sample
  efficiency.
\end{chapterabstract}
...
learning are raising hope for predictive modeling in healthcare. The richness of
the data in EHRs and their integration in routine care make them a promising
application area \citep{raghupathi2014big}.
