...
%
\paragraph{Substantial efforts are engaged to collect routine care data for reuse}

Vast amount of new data is collected in healthcare at the cost of new
infrastructures. Navigating through this mass of data is greatly simplified,
mostly by the centralization of data fragmented in non interoperable softawares
and NLP techniques. Mainly aiming to improve clinical research, deriving new
evidences from EHRs is currently freined by a poor normalization of data access
processes, heterogeneity of local practices, and unexhaustive collection of
critical elements of the healthcare trajectories even during hospital care.

\paragraph{These data repositories are not big enough} Even the regional scale
data center of the AP-HP has a small number of cases for common pathologies such
as cardiovascular adverse events as \ref{chapter:predictive_models} shows. On
these medium sized datasets, flexible models on top of simple data
representations --such as random forest with event counts-- are performing
better than transformer-based models while being simpler to be deployed.

\paragraph{Big data is no magic trick, we need causal thinking} Size of data put appart,
the observational nature of EHRs data require clear and robust workflows
inspired by modern causal inference. Using flexible predictive models without a
causal framework opens the door to collider biases, but using only
linear models is prone to underfitting and biased estimates as well.
...
\paragraph{Could design model that optimize the R-loss directly?}

%There is a paper from the chernozukhov
% clics that does that : rieznet I think. DIcuss the relation with the
% two-headed neural networks studied by Johansson et al.

\paragraph{How to explain the high inter-experience variance for model selection?}
% Overlap is a component but there 

\paragraph{The potential of Large Language Models for prediction from EHRs}
% 

\section{Personal convictions for practical perspectives}
...
\paragraph{The unreasonable effectiveness of healthcare data is yet out of
  reach} Due to the impossibility to transfer models or administrative barriers to
access data (due to multiplicity of the involved actors), predictive models
deployment is almost unexistant. \md{source PA pour le non remboursement des DMs
  avec IA.} This forces us to rely on efficient techniques that make the best of
medium-sized data or rely on sharable sources of knowledge. This calls for more
work in research network methodologies \citep{hripcsak2015observational} and
model sharing such as federated learning, and trained estimators.

%
Ultimately, I am convinced that resource constraints and the ever-increasing
demand on healthcare bridges the care effectiveness question to equity. Some
patients will always be less treated than others: public health should seek for
random repartition of these mistreated patients conditionally on patient
conditions.
