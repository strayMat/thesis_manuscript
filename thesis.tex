...
\subsection{\textit{Data looking for a question [...] and rais[ing] puzzles}, David Cox}%
...
researchers foresaw a huge potential for improving healthcare in applying
...
\begin{background_box_left}
  ...
  one precise research question. RCTs at the core of the \emph{evidence based
    medicine}, that supersede randomized experiments over natural experiments
  --uncontrolled-- or expert opinions in a hierarchy of evidence
  \citep{guyatt1995users}. Their methodology has been shaped the large-scale
  International Studies of Infarct Survival (ISIS) trials
  \citep{isis_randomised_1986}. Two other research data collections are cohorts
  ...
  % Recent surges in data collection encourage deviation from the standard
  % interventional design by relaxing some of the methodological constraint of RCTs.
  % However, HTA agencies witness a deteroration of evidences for new drugs and
  % strongly advocate against observational studies for replacing new drugs
  % evaluation \citep{wieseler2023replacing,vanier2023rapid}.
  %   % other 
  ...
  sources of RWDs are insurance claims and EHRs.
  ...
  United States 80\% of hospitals \citep{adler2017electronic} and close to 90\% of office-based practices
  have now adopted EHRs \citep{office_adoption_2023}.  Used routinely by clinicians, EHRs enable them to record
  and interrogate clinically relevant information for patient care mainly
  through notes. EHRs can be specific to an institution, or shared between
  several actors \citep{hoerbst2010electronic}. This central system is
  accompanied by other business applications, such as patient administrative
  management, computerized prescribing, biology software, reanimation software,
  and imaging software. Together, these softwares make up the Hospital
  Information System (HIS). Depending on the degree of maturity of the HIS, the
  various data sources communicate more or less well with each other.
  ...
  for RCTs and RWD is the mechanism of treatments allocation. In interventional
  data such as RCTs, researchers focus on a precise intervention of interest.
  Their goal is to measure the effectiveness of this intervention with respect
  to a patient outcome --eg. mortality. The probability of a patient to receive
  the treatment under study is known --often set to 50\%. It is independent from
  the patient characteristics. This enforces the comparability between treated
  patients and control patients --not receiving the treatment. Therefore, once
  this randomization has been repeated over many patients, \emph{on average},
  the difference in outcomes can only be attributed to the treatment. 
  ...
  true experiment.} Epidemiological textbooks clearly  since randomization
  yields excellent internal validity \citep{campbell1957factors}: the estimated
  treatment effect is close to the true treatment effect if we could repeat the
  experience indefinitely on the same population --the estimate is
  \emph{unbiased}. The RCT methodology requires almost no statistical hypothesis
  \citep{colnet2020causal}, and the measurements of only two variables: the
  treatment and the outcome. This contributed to its success for clinical
  evidence generation.

  \md{Why observational data does not have randomization?}

  % threat to external validity
  \paragraph{External validity: when RCTs do not inform}

  Interventional data is hard to collect: patients must be voluntary, without
  comorbidities. These factors endanger the external validity of
  interventional data \citep{feinstein1997problems,concato2000randomized,rothwell2005external}: The findings of a study may
  not apply to real world situations since the population and experimental
  conditions differ too much from usual practices.
  ...
  situation is closer to laboratory settings. This difference in data sources and
  methodology is visible in the divide between clinical and social epidemiology
  \citep{}. What about medico-economics ? I do not address this question in the
  present thesis, but it is a clear motivation.\md{need a citation here}
  ...
  heterogeneity of treatment among population subgroups is of great concern in
  epidemiology and healthcare \citep{hernan2020causal} and key for personalized
  ...
  topol2019high}. In sequential decision making problems, heterogeneity of
  treatment effect is close to subfields of the Machine Learning community such
  as operational research \citep{schaefer2004modeling} or reinforcement learning
  \citep{bareinboim2015bandits}. These fields are focusing on sequential
  ...
  in the large dimensional state-action space, those work often require a
...
cultures: a predominant community at the time focused on models, and an emerging
trend that rely only on predictive accuracy.%

Epidemiology and biostatistics favor the model-based culture as one can judge
from textbooks \citep{rothman2012epidemiology}: \textit{In many epidemiological
  applications, it is the choice among effect measures that dictates the type of
  model the investigator ought to use.} Because, clinical epidemiology seeks to
uncover nature mechanisms from data, it favors linear models that make explicit
simple links between variables.

%However these models are often based on strong assumptions that are debated in practice.
%For example, the assumption that parameters in linear models are interpretable \citep{lipton2018mythos}.

On the contrary artificial intelligence in medicine has a more nuanced approach
to models. One of the main goal of AIM is \textit{to provide expert medical
  consultation to general practitioners or paramedical personnel in communities
  where such consultation is normally unavailable}. This practical goal balances
...
% AIM adopts a more pragmatic approach, trying to understand what deployed
% healthcare system is improving outcomes or not, without trying to uncover the
% full causal mechanism leading to the observed results.
...
\begin{background_box_left}
  ...
  boosting, neural networks or Support Vector Machines. Appendix
  \ref{apd:intro:ml_algorithms} details the principles of random forest and
  boosting since we are heavily using them in this thesis.
...
\section{Which questions matter in healthcare?}\label{sec:intro:causality}

\subsection{What pressing needs to use health data}

\paragraph{The promise of Real world data}

On the one hand, the use of this data translates into direct benefits --primary
uses-- for the patient by serving as the cornerstone of the developing
personalized medicine \citep{mann_artificial_2022,ziegler2022high}. They
also bring indirect benefits --secondary uses-- by accelerating and improving
knowledge production: on pathologies \citep{campbell_characterizing_2022}, on
the conditions of use of health products and technologies
\citep{safran_toward_2007,tuppin_value_2017}, on the measures of their safety
\citep{wisniewski_development_2003}, efficacy or usefulness in everyday
practice \citep{richesson_electronic_2013}. They can also be used to assess
the organizational impact of health products and technologies
...
\subsection{One choice of perspective: Recent statistical
  learning for EHRs}\label{subsec:intro:focus_data}

In this thesis, we focus on Electronic Health Records even if some of our
results could be relevant for insurance claims as well.
...
% stating my questions
\subsection{Are flexible statistical models a valid tool to answer today's
  questions in healthcare?}%
\label{subsec:intro:questions}%

% justification on why the data it is adapted to the questions (mentioned earlier)
...
Context of steady increase in healthcare costs \citep{mcginnis2013best,
  data_oecd_health_spending}.
...
% maybe move to what questions are asked in healthcare ?
...
healthcare. Which predictive models are useful ? Is prediction enough ? How these
...
health \citep{cma_policy_appropriateness_2015}.

\subsection{Contributions}\label{subsec:intro:contributions}

Chapter \ref{chapter:cdw}.%
