...
\paragraph{Claims data are not the solution for every questions, but should be
  used more for public health} They are well normalized, well documented, lack
few critical PICO elements \citep{caruana2023administrative}. Despite some
difficulty to define precise populations in some pathologies, they are an
interesting source for studying treatment effectiveness and public health
policy at the population level. Estimations methods borrowed to econometrics
such as difference-in-difference \citep{athey2006identification}, regression
discontinuity design \citep{bor2014regression} and instrumental variables
\citep{greenland2000introduction} should be investigated as they might be
adapted for specific questions.
...
deployment is still rare. This encourages us to rely on saple efficient
...
leverage large language models pretrained on language to either extract
information from clinical notes, or directly for predictive tasks
\citep{jiang2023health} --at the condition to have sufficient computing
...
\ref{fig:causal_diagram_albumin} become tedious. Even if all information is
...
non-text data. Vibration analysis confronting these methods is needed, eg. by
replicating on text the analysis of chapter \ref{chapter:causal_tuto}.

%
\paragraph{Modern public health problems should take into account the fragmented
  nature of healthcare} Modern healthcare burdens and costs are driven by
  chronic diseases where death or rehospitalization are not the only outcomes of
  interest. Most of care is provided outside of the hospital
  \citep{white1961ecology}, therefore requiring data linking between 



Public health interventions should focus on smaller
rewards, for which experiments are easier to conduct and where error is more
acceptable. Adopting methods from adapative would allow better evaluation and
continuous improvement for interventions since we can repeat more experiments.
and possibly high delayed reward because of better prevention.
...
should seek for a random repartition of these mistreated patients conditionally
