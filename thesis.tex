...
At the end of the master, continuous improvements in ? until Bert paper.

\paragraph{Billing claims: an underused huge pile of data}

First questions: how useful are such wealth of data ? How to use it ?

Covid-19: National healthcare monitoring during the epidemics. Integrated data
pipeline are very useful during crisis for monitoring

\subsection{Increasing collection of data and computing power}

A whole generation of quantitative researchers has been trained with flexible
data-hungry models.

The unreasonable effectiveness of data \citep{halevy2009unreasonable}.

\subsection{A pressing need for using the collected data}
...
Embraced by artificial intelligence in medicine community. How useful is a model
?



\subsection{Epidemiological Framework}

\subsection{One choice of perspective: Recent statistical
  learning}\label{subsec:intro:recent_statistical_learning}
% 
Learning statistics in the age of deep learning push towards adopting a view on
  statistics on considering than mechanisms are in general unknown ant that
  predictive accuracy is the one and only mean to evaluate how a model fit to
  data \citep{breiman2001statistical}.
% 
% ML in healthcare 
