...
predict an image label from raw pixels learns internal representations of
natural images such as feet, faces, or hands. These intermediary representations
can be reused --transferred-- to applicative tasks with different objectives
such as predicting the severity of a traumatism from a photo. This paradigm
--called pre-training-- has been very successful in applied domains such as
computer vision \citep{krizhevsky2012imagenet}, then Natural Language
Processing (NLP) \citep{devlin2018bert} and structural biology
...
the fascination for deep learning left me wonder --as many other students at
this time-- in which other areas those techniques could be successfully applied.
...
\md{Find a better title for this part ?}
...
\begin{background_box_left}
  ...
  medicine}, that supersede randomized experiments over natural --non
  randomized-- experiments or expert opinions in a hierarchy of evidence
  ...
  \paragraph{More data collected routinely: Real World Data}\label{subsec:intro:real_world_data}

  % definition of routine care data
  Healthcare data is increasingly collected in from electronic information
  system used in routine care
  ...
  Two major sources of these Real World Data (RWD) \citep{fda_real-world_2021} are
  claims and EHRs.

  \paragraph{Billing data: claims}\label{def:claims}
  ...
  background or reason for seeking care \citep{ziegler2022high}. Finally, billing optimization processes
  endanger the validity of billing variables by over-representing
  well-reimbursed cares over \citep{juven2013codage}.

  \paragraph{Clinical data: EHRs and Hospital Information System}\label{def:ehr_his}

  The computerization of paper-based patient records led hospitals to adopt EHR
  solutions beginning in the 1990s. These are defined as the longitudinal
  collection of health data in an electronic information system (IS)
  \citep{gunter2005emergence}. Used routinely by clinicians, EHRs enable them to
  record and interrogate clinically relevant information for patient care mainly
  ...
  \citep{butte2006creation,patel2009coming}. Since then, the wide dissemination
  of EHR and the increasing richness of their content are fuelling new research
  avenues for AIM \citep{yu2018artificial}, clinical research
  \citep{cowie2017electronic} and epidemiological studies
  \citep{casey2016using,gianfrancesco2021narrative}. In this thesis, we focus on Electronic
  Health Records even if some of our results could be relevant for insurance
  claims as well.
...
personalized medicine \citep{mann_artificial_2022,ziegler_high_2022}. They
...
\subsection{One choice of perspective: Recent statistical
  learning for observational routine care data}\label{subsec:intro:focus_data}
