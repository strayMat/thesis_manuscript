...
\begin{background_box_left}
  ...
  \md{Develop a toy example of confounded variable here without a prediction model}
...
Are flexible statistical models a valid tool to answer today's questions in healthcare?
...
well-understood tasks to seek care tailored to each individual, even if the
mechanisms of action are poorly understood.
\citep{mann_artificial_2022,ziegler2022high}.
...
to the main goal of epidemiology which is \textit{to study of the distribution
...
data to develop new evidence on the effectiveness of interventions
\citep{richesson_electronic_2013} \md{develop: schneeweiss and the replicate
  program}.
% % too much
...
\paragraph{Public health issues: old and new}
...
\citep{cma_policy_appropriateness_2015}. Scientific associations issue medical
guidelines built upon the scientific literature to recommend ideal care
trajectories. Relying on a hierarchy of evidence \citep{guyatt1995users}, these
recommendations are strongly relying on RCTs \citep{}. In the United States, the
Committee on Learning Healthcare System suggested to better use routine data to
adapt evidence to real practice \citep{mcginnis2013best}. \md{develop this part}
...
% Initial motivation: public health guideline, confront them with EHR data ? Do we need predictive models and
% which one ?
\md{Here or latter ?  I want to explain that causation is at the hear of this
  thesis, but that we did came here because of the public health question}
...
\begin{background_box_left}
  ...
  \label{subsec:intro:causation_epidemio}%
  ...
  % even AIM is rooted in causality 
...
evaluating the effectiveness of clinical guidelines using Electronic Health
Records. Which predictive models are useful ? Is prediction enough ? How these
increase in predictive power can serve the true objective of healthcare:
delivering appropriate treatment to each and every patient to improve her health
\citep{cma_policy_appropriateness_2015}?
...
\begin{itemize}
  ...
  \item Chapter \ref{chapter:predictive_models},
...
\md{summary of chapters: abstract?}
