...
\begin{chapterabstract}[Abstract]
  ...
  (MIMIC-IV). In high-dimensional settings, the selection of hyper-parameters
  for the causal model is crucial to avoid under- or over-learning. In a
  simulation and three semi-simulated datasets, we shows that the doubly robust
  R-risk outperforms other existing causal risks.
...
\begin{chapterabstract}[Résumé en français]
  ...
  certain nombre de défis pour leur utilisation efficace. Pour remédier à ces
  ...
  Dans des problèmes à grande dimension, la sélection d'hyperparamètres pour les
  modèles causaux est cruciale pour éviter le sous-apprentissage ou le
  sur-apprentissage. Pour une simulation et trois ensembles de données
  semi-simulées, nous montrons que le risque R doublement robuste surpasse
  d'autres risques causaux existants.
\end{chapterabstract}
\vfill
...
côtés. J'espère à l'avenir, faire honneur à cette formation. Merci à Claire
d'avoir été là. Ca n'a pas été facile de garder contact à distance, mais nos
discussions et tes conseils ont toujours été précieux.\\
...
MIND -- SODA/scikit-learn. Merci à tous les PI et post-docs: Marine, Judith,
...
du jeudi et les chouquettes: Romain, Thomas, Perceval, Ariel, Adam, Charline, Etienne,
...
à tous ceux de Saint-Ex avec qui je m'échappe le moment d'un WE: Dédé, Paupaul,
...
Noémie. Merci Orel pour ces séances de grimpe à Pantin. Merci Philou pour les
grands moments de discussion. \\

Papa, Maman, Cécile, vous m'avez accompagné depuis les premiers tintins jusqu'à
l'école d'ingénieur et la thèse. Je sais ce que je vous dois et j'espère ne pas
l'oublier. Merci pour votre présence et pour le goût de la lecture, qui mène si
loin. \\
...
\begin{chapterabstract}\label{sec:predictive_models:abstract}%
  ...
  multiple categories of data sparkling interest in the development of
  predictive algorithms. Text and image put apart, this data can
  be represented as time-varying features with high cardinality encoded in medical codes.
...
number of cases (see a review of number of cases for three major transformer
based models in Appendix \ref{apd:foundation_model_nb_cases}). Due to the
difficulty to access data linked to privacy rights enforced by the General Data
Protection Regulation (GDPR), we should evaluate prognosis models on smaller
samples.

% Major dataset shifts
\paragraph{Transferring predictive pipelines might not be effective} A potential
solution to local small sample sizes is to transfer models pretrained on large
populations. However, privacy requirements, once again prevent such transfer
...
Even if publically available, major dataset shifts might break effective transfer
...
\md{mieux distinguer la frugalité en données et la frugalité en ressources. Etre
  plus cash sur la situation dans le monde entier avec les ressources de calcul
  et la course aux GPUs. Et plutôt le tourner en positive.}
\md{Faire apparaitre commoditty hardware.}
...
\paragraph{Prediction tasks framing} General workflows describe key steps to
develop predictive models with good practical utility
\citep{ohdsi2019book,tomavsev2021use}. Here, we detail our framework to
precisely define each task. First, we define a \emph{study period} during which
data acquisition was sufficiently stable. Then, we define and select the
\emph{cohort} detailed for each task. Inclusion criteria are detailed in the
flowcharts of Figure \ref{apd:fig:flowcharts}. We define the task outcome with:
an \emph{index visit}, an \emph{observation period} during which events are fed
to the predictive model, an \emph{horizon} after the end of the observation
period defining the followup, and a target event potentially occurring during
the followup.

The more clinically anchored a task is, the less appropriate it is for
calibration models, as local practices will be more important in
...
We split each cohort described in Table
\ref{table:predictive_tasks_summary} into a train and a test set
...
\subsection{Results -- Tree-based models on event counts prevail}%
\label{subsec:predictive_models:results}


\paragraph{Count encoding followed by random forest outperform elaborated
  embedding models}%

Both for the prognosis task (shown if Figure \ref{fig:prognosis_auprc}) and the
...
is far from the best performance, certainly because of the small samples.


% lienar combination of icd10 chapter is a strong baseline
Interestingly for prognosis, the performances of logistic regression with
demographic is already very good. This indicates that a simple linear
combination of the previous stay chapters is a strong baseline.

\begin{figure}[!h]
  \centering
  \includegraphics[width=\linewidth]{img/chapter_3/prognosis/average_precision_score__c_weighted.pdf}
  \caption{Prognosis average precision score, weigthed by prevalences over 21
    ...
    average precision rather than ROC AUC, since it takes better into account
    the difference in prevalence between chapters. Appendix
    \ref{apd:temporal_split:prognosis} details average precision scores for each
...
performances on the prognosis task plotted against the prevalence. Low
prevalences translate into small number of cases, which decreases overall
performances and increase variances among chapters with less than 5\%
prevalences. Appendix \ref{apd:prognosis_prevalences} shows similar results for
ROC AUC and linear estimators.
...
different pipelines average over the 21 chapters on the prognosis task,
...
\begin{figure}[!h]
  \begin{minipage}{.35\linewidth}
    \caption{Static embeddings are quicker to train. The training times are
      ...
      memory.}\label{fig:prognosis_training_time}
  \end{minipage}%
  \hfill%
...
models are hindered by the challenge of publicly sharing pre-trained models
\citep{wornow2023shaky}. Training them from scratch requires important computing
resources, and access to very large cohorts. We explored a
...
For simple predictive tasks such as LOS or prognosis at the ICD10 level and
...
disposal. Our benchmark also lacks a pre-trained model on large data and the
intersting avenue of repurposing large language model for predictive tasks
directly from clinical notes. These two approaches need large GPU resources,
making a poor use of the comodity hardware already present in hospitals.
Healthcare financial resources are already scarse in rich countries. It is still
an open question to assess if the prevention benefits brought by large scale
models from EHRs outweight the cost of their deployment.
...
interoperable softwares and NLP techniques. The main aim of these CDWs is to
...
performed with the classical mean squared error. Estimating nuisance parameters of
...
\citep{jiang2023health}. These perspectives require having sufficient
...
\paragraph{For treatment effect estimations, relying on text calls for better
  understanding on causal representations learning}

Using text for confouder adjustment opens many research question when looping
back to causal thinking. Building a causal graph as in Figure
\ref{fig:causal_diagram_albumin} becomes tedious. Even if all information is
present in the text, should we extract every variable or is it possible to build
appropriate representations? Optimizing directly the R-loss by gradient descent
could be an interesting avenue, which is closely related to the work of
\cite{johansson2022generalization} and \cite{chernozhukov2022riesznet} with
non-text data. Vibration analysis is needed to confronte these methods on
applications such as the one presented in chapter \ref{chapter:causal_tuto}.
...
populations make claims inadaptated to precise clinical questions, but they
...
variables \citep{greenland2000introduction} might be adapted.
...
socio-economics features to hospital data. This would allow to study less
...
benefits.
%
Ultimately, I am convinced that the resource constraints face by modern
healthcare systems bridges the care effectiveness question to equity. Some
patients will be less treated than others but public health should seek for a
random repartition of these mistreated patients.
...
\begin{appendices}
  ...
  Figure \ref{fig:prognosis_rocauc} displays the area under the precision
  ...
  Figure \ref{fig:prognosis_auprc_macro} displays the area under the precision
  recall curve (average precision) averaged with equal weight for each chapter (Macro).
  \begin{figure}[!h]
    \centering
    ...
    \caption{Prognosis Average Precision averaged by prevalence over chapters
