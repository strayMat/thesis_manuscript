...
\begin{chapterabstract}[Abstract]
  ...
  Chapter \ref{chapter:intro} details the current tension between these new
  sources of data, machine learning and modern public health issues. This
  motivates the main research question of this work: How flexible models can
  help delivering appropriate treatment to each and every patient to improve her
  health?

  Chapter \ref{chapter:cdw} summarizes semi-structured interviews from a
  national case study of the Clinical Data Warehouses (CDWs) of the 32 French
  regional and university hospitals. It highlights key aspects of this
  cornerstone infrastructure for the collect and analysis of Electronic Health
  Records (EHRs).

  Acknowledging the difficulty to access large sample sizes and computational
  power to develop generalizable predictive models, chapter
  \ref{chapter:predictive_models} explores a complexity gradient in
  representation and predictive algorithms for EHRs.

  Anchored in causal thinking, chapter \ref{chapter:causal_tuto} details
  key elements necessary to robustly estimate treatment effect from time-varying
  EHR data. It illustrates various possible variations in studying the effect
  of albumin on sepsis mortality in the Medical Information Mart for Intensive
  ...
  % care.
  In high-dimensional settings, the selection of hyper-parameters for the causal
  model is crucial to avoid under- or over-learning. Chapter
  \ref{chapter:causal_model_selection} shows that the doubly robust R-risk
  outperforms other existing risks in a simulation and three semi-simulated
  datasets.
...
\begin{chapterabstract}[Résumé en français]
  ...
  l'organisation routinière des soins. Ces enregistrements des soins quotidiens
  ouvrent la porte à de nouvelles questions de recherche, notamment en ce qui
  concerne la qualité des soins, l'efficacité des interventions après leur mise
  sur le marché, l'hétérogénéité de leurs bénéfices dans les populations mal
  desservies ou le développement de traitements personnalisés. D'un autre côté,
  la complexité et la nature à grande échelle de ces bases de données posent un
  ...
  Le chapitre \ref{chapitre:intro} détaille la tension actuelle entre ces
  nouvelles sources de données, l'apprentissage automatique et des
  problématiques modernes de santé publique. Cette tension motive la principale
  question de recherche de ce travail : Comment des modèles flexibles
  peuvent-ils aider à fournir un traitement approprié à chaque patient afin
  d'améliorer sa santé ?

  Le chapitre \ref{chapter:cdw} résume les entretiens semi-structurés menés dans
  le cadre d'une étude de cas nationale portant sur les entrepôts de données
  cliniques des 32 hôpitaux régionaux et universitaires français. Il met en
  lumière les principaux aspects de cette infrastructure essentielle pour la
  collecte et l'analyse des dossiers patients informatisés (DPI).
  ...
  le chapitre \ref{chapter:predictive_models} explore un gradient de complexité
  dans les représentations et les algorithmes prédictifs sur DPI.

  Ancré dans la pensée causale, le chapitre \ref{chapter:causal_tuto}
  détaille les éléments clés nécessaires pour estimer de manière robuste l'effet
  du traitement à partir de données de DPI variant dans le temps. Il illustre
  diverses variations possibles dans l'étude de l'effet de l'albumine sur la
  mortalité dans des cas de septicémie avec la base de données MIMIC-IV (Medical
  ...
  sur-apprentissage. Le chapitre \ref{chapter:causal_model_selection} montre que
  le risque R doublement robuste surpasse d'autres risques existants dans une
  simulation et trois ensembles de données semi-simulées.

  Le chapitre \ref{chapter:conclusion} examine des pistes potentiellement
  intéressantes pour améliorer le développement et l'évaluation d'interventions
  adaptées, de politiques de santé publique ou d'indicateurs de qualité des
  soins.
\end{chapterabstract}
\vfill
...
\gv{Small populations: no silver bullet. Il faut qu'on parle d'abord de petites
  ...
  aux faibles number of cases.} \subsection{Transfer learning: no silver bullet}%
...
\md{Améliorer la figure: eg. plus de densité, milliers de patients en x plus lisible.}
\begin{figure}[!h]
  \centering
...
\md{Changer par la weigthed average. }
\md{Piste d'amélioration: détailler le cost effectiveness des méthodes. Proche
  de public health, qu'est ce qu'on y gagne opérationnellement? Pour ça voir si on
  a des références pour mace ou pour un chapitre icd10}
\begin{figure}[!h]
...
experiments are easier to conduct and where error is acceptable: it would allow
better learning for decision making since we can repeat more experiments, and
possibly high delayed reward because of better prevention.
...
\begin{appendices}
  ...
  \begin{figure}
    \centering
    \includegraphics{img/chapter_1/tree_3.pdf}
  ...
  \begin{figure}
    \centering
    \includegraphics{img/chapter_1/forest_4.pdf}
  ...
  boosting \citep{freund1995desicion} is a method that iteratively sums weak
  learners --typically trees-- to each other. Each new weak learner improves
  over the errors of the previous averaged estimators. Improvement on errors can
  be performed by solving analytically the error loss function or numerically by
  gradient descent \citep{friedman2001greedy}. The later has the advantage to
  avoid a new implementation for every new loss function. The final estimator
  has the same form than a random forest, but require substantially less trees
  to converge. Figure \ref{fig:boosting_regression} illustrates on a toy example
  of boosting trees with increasing number of trees of depth 4.

 \begin{figure}
    \centering
    \includegraphics{img/chapter_1/boosting_4.pdf}
