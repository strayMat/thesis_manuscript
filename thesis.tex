...
\begin{background_box_left}
  ...
  \textcolor{blue}{the administration of a drug for treated patients} compared to
  \textcolor{orange}{no administration for control patients}. In observational data, interventions
  are far from random but focused on patients requiring care. In observational
  data --illustrated in Figure \ref{intro:fig:observational}, the drug would be given to patients with
  ...
  % interventional  
  For interventional data --shown in Figure \ref{intro:fig:interventional}, the
...
\begin{figure}[h!]
  \begin{subfigure}[b]{0.5\linewidth}
    \centering
    \includegraphics[width=\linewidth]{img/chapter_1/sample.pdf}%
  \end{subfigure}%
  \caption{}\label{intro:fig:observational}
  \begin{subfigure}[b]{0.5\linewidth}
    \centering
    \includegraphics[width=\linewidth]{img/chapter_1/sample_rct.pdf}%
    \caption{}\label{intro:fig:interventional}
  \end{subfigure}%
  \caption{\ref{intro:fig:observational})In routine care, treatments allocation
    is not random: priority is often given to patients that will benefit the
    most from the intervention. \ref{intro:fig:interventional}) In
    interventional studies, randomization ensures the comparability of treated
    and controls on average.}\label{intro:fig:observational_vs_interventional}
\end{figure}
...
\subsection{Model-based statistics: Experts know the structure of the problem}%
...
parameters as valid nature constructs \citep{cox2001statistical}. Because
...
Assuming the explicit relation between variables in linear models is tempting
and quickly allow to draw conclusions from data. But interpretability of the
model parameters is a strong assumption \citep{lipton2018mythos}, which relies
on the well-specification of the model: The underlying variables and their
relations built into the models should describe \emph{real} mechanisms.

% choice of correct models and interpretability are strong assumptions
Despite judging the model choice as \textit{crucial to fruitful application}
\citep{cox2006}, this tedious task is left to experts, that should have an
appropriate theoretical model of the problem \citep{cox2001statistical}. This
habit is prone to circular reasoning where a model is chosen only to justify a
pre-specified theory. Crowdsourcing studies asking the same question to
...
\begin{background_box_left}
  ...
  boosting since we are using them in the different chapters of the thesis.


  seeking algorithm efficiency and out-of sample predictive accuracy.

  % ref to appendix to describe ML algorithms: at least ridge, random forest,
  % boosting.

  \paragraph{Model selection: how to choose bewteen multiple potentially good models?}
...
Artificial intelligence in medicine has a more nuanced approach to models. One
of the main goal of AIM is \textit{to provide expert medical consultation to
  general practitioners or paramedical personnel in communities where such
  consultation is normally unavailable}. This practical goal balances between good
predictive performances --low error rates-- and adoption by practitioners. The
latter built upon trust, an aspect of interpretability that increases when
estimator error cases are the same as human errors \citep{lipton2018mythos}.

For high dimensional data such as EHRs, linear models are not strictly more
interpretable than deep neural network \citep{lipton2018mythos}.
