...
\begin{titlepage}
  ...
  \begin{picture}(0,0)
    ...
    \put(-110,-743){\rotatebox{90}{NNT: 2023UPASG073}}
  \end{picture}
  ...
  \Huge Representations and inference from time-varying routine care data \\

  \normalsize
  \color{black}
  \Large{\textit{Représentations et inférence à partir de données de santé temporelles collectées en routine}}
  %*****************************************************
  ...
  \footnotesize Thèse préparée dans l'unité de recherche Inria Saclay-Île-de-France
  (Université Paris-Saclay, Inria), sous la direction de \textbf{Gaël
    Varoquaux}, directeur de recherches à l'Inria, le co-encadrement de
  \textbf{Claire Morgand}, Dr à l'Agence Régionale de Santé Ile-de-France, et la
  co-supervision de \textbf{Pierre-Alain Jachiet}, Dr. à la Haute Autorité de Santé
  \vspace{15mm}

  \textbf{Thèse soutenue à Paris-Saclay, le 20 novembre 2023, par}\\
  \bigskip
...
discussions passionnantes que ce soit sur les plans scientifiques ou juste de
...
supporté ces trois dernières années. Merci à Pilou pour m'avoir donné le goût
...
ami attentif. Merci à ceux que je vois de loin en loin: Féf, Anthoine, FH,
Paul, Sylvain, Hippolyte, Manu, Gustave.\\

Merci Nathalie de m'avoir fait visiter il y a longtemps ton labo de biologie,
alimentant ma curiosité pour la recherche.
...
There is currently a tension between massive routine care data collections such
...
\begin{background_box_left}
  ...
  The gold standard for generating new evidence in healthcare are Randomized Controlled
...
\begin{background_box_left}
  ...
  simulated environment \citep{bennett2013artificial}. Simulations are
  convenient to
...
\begin{background_box_left}
  ...
  comparing their performance on an unseen test dataset. If we were to evaluate
  ...
  selected on the average performance over the K test sets.
...
\begin{background_box_left}
  ...
  a model, and hence to the measure of performance for relevant downstream
  tasks. For other domains such as NLP, consensual downstream tasks such as text
  classification have been established to evaluate the performance of
...
\citep{jiang2023health}. Hope is high that LLM models will soon be able to help
...
deterioration of evidence for new drugs and strongly advocate against
...
\begin{background_box_left}
  ...
  example from \cite{murphy2022probabilistic_chapter36}: \textit{Consider three
    ...
    incorporate some causal knowledge of the world — \eg which of these scenarios
    is plausible?}
...
chapter study the performance of five causal risk to select an outcome model
...
l'inférence causale à partir des DPI, examinons trois exemples concrets.
...
problème, connu sous le nom de \emph{malédiction de la grande dimension} a été
introduit en recherche opérationnelle \citep{bellman1957dynamic} et est
bien connu en statistique \citep{breiman2001statistical} : Le nombre
...
séquences d'événements de facturation peuvent être considérées comme des
...
\citep{beam2019clinical}. Les relations entre les événements capturées par ces représentations
...
services publics cherchent à comprendre l'hétérogénéité des consommations de
...
bonnes pratiques en santé à partir des données de vie réelle. Quels sont les
...
de ce chapitre contraste avec le contexte général de la thèse, plus mathématique.
...
needed to pool data from one or more medical information systems --whatever
...
research. Appendix \ref{apd:cdw:table:expert_teams} lists all interviewed people with their team title.
...
website of the hospital data warehouse.  \footnote{The full collection of ongoing study is available on this url:
  \url{https://gitlab.has-sante.fr/has-sante/public/rapport_edsh/-/blob/master/data/cycle_eds/cycle_eds_etudes.csv?ref_type=heads}}
...
circuit (prescriptions and administration, 16/21), Intensive Care Unit (ICU, 2/21)
...
characterization (25 \%), followed by the development of diagnostic and prognostic
algorithms (24 \%), the study of risk factors (19 \%) and the treatment effect
evaluations (15 \%).
...
\paragraph{Multidisciplinary teams} As the CDW becomes an essential component of data management in the hospital,
...
\paragraph{Lack of sustainable funding} The resources specific to the warehouse are rare and often taken from other
...
\paragraph{Poor adoption of common standards} The more heterogeneous a HIS
system is, the poorest quality will have a CDW built above it. There is a need
for increasing interoperability, to help EHR
...
\paragraph{Lack of translational researches} Many ongoing studies concern the
development of diagnostic and prognostic algorithms whose
...
\subsection{Data quality and documentation: more incentives needed}%
...
\citep{de2022guidelines}. We explore a complexity gradient of predictive
...
preprocessing usually requires mapping non standard terminologies, create derived
...
familiar with the complexity of the full EHR data processing pipeline. We propose to
...
prediction model deployed in hundered of US caresites. The model transfer
performance dropped from 0.76 to 0.83 area under the receiver operating curve
(ROC AUC) declared by the manufacturer to 0.63.
...
effectiveness. Improving the strength of this evidence requires
repeatedly testing these systems on different prospective populations with intuitive metrics
...
healthcare processes. To benefit the largest number of patients accross the
...
There is a need to balance good predictive performance with model frugality.

\subsection{Objective and outline of the paper}%
\label{subsec:predictive_models:contribution}%

\paragraph{Contributions}
We study the performance of increasingly complex models on three operational
...
effective. We propose a new model based on the transfer of static medical
...
\ref{subsec:predictive_models:expe_definitions} defines three clinical tasks
...
better performance than transformer-based models on general tasks such as
...
solution. We quantify the detrimental effects on prognosis performance of
...
\subsection{Static embeddings of event features: $g_{emb-local}$ or $g_{\mathrm{emb-SNDS}}$}
...
cooccurrence matrix between medical concepts. It creates neighboring vector
representations for codes that coocur frequently together and thus induces
...
for decayed counting and local embeddings: $g_{\mathrm{emb-SNDS}}((S, (0, \delta))) = [C_{0} \cdot \Phi_{SNDS}, C_{\delta} \cdot
  \Phi_{SNDS}]$.
...
in the flowcharts of Figure \ref{apd:fig:flowcharts}. We define the task
...
\begin{figure}[!t]
  ...
  \caption{Prognosis AUPRC, weighted by prevalences over 21
    ICD10 chapters with more than 1\% prevalence. The performance is averaged
    ...
    stay codes for the target stay. Random forest have better performance.
...
the horizontal black line. The good performance of the logistic regression with
...
\begin{figure}[!h]
  ...
  \caption{Length of stay ROC AUC for different featurizers and estimators. The
    performance is averaged over 5 folds. The shaded area represents the
...
decreases overall performance and increase variances among chapters with less
...
embeddings followed by random forest are sufficient to reach good performance.
...
access. However, the type of these resources are different. Static embeddings
can reach satisfying performance on big samples with large memory consumption
...
could improve the performance for all tasks. However, the addition of these
...
are typically measured by Randomized Controlled Trials
(RCTs).
...
\eg a patient either receives mechanical ventilation or not during admission to an
intensive care unit compared to, \eg blood transfusion which may be given repeatedly.
%
...
intervention, \eg prescription for a
...
\begin{background_box_left}
  ...
  covariates set generally reinforces the ignorability assumption. On the
...
\begin{background_box_left}
  ...
  while for binary outcomes (\eg events)  the choice of estimand depends on the
  scale of the study. Whereas the risk difference is very informative at the
  population level, \eg for medico-economic decision making, the risk ratio and
...
indirectly, \emph{\eg} via billing codes, the time-wise dimension requires
...
many diverse individuals. EHRs and claims data are well suited
for these models, as they easily cover more individuals
...
care provider. Yet, if the EHR data does not contain this information, it
...
Our framework suggest integrating the evidence from RCTs designed with
...
\begin{chapterabstract}
  ...
  observed data. But does estimating these nuisances add noise to model selection?
  % Contributions
...
asymptotically large data. These works rely on introducing a
...
\begin{background_box_left}
  ...
  treatment or no treatment, called \emph{response function}:
  \begin{flalign*}
  ...
  finite sum version of equation \ref{eq:tau_population} leads to an
  ...
  $\varepsilon$ is residual noise of mean zero.
...
identification (equation \ref{eq:mu_identification}), a given outcome model $f: \mathcal X
...
equation \ref{eq:causal_model_selection}) may involve estimating various quantities
...
explicit the role of overlap for the performance of causal risks.
...
and is not appropriate to discriminate between models with close performance.
...
the strong performance of $R\text{-risk}$ in randomized setups
...
boosting trees for both the response surfaces and the treatment\footnote{Scikit-learn regressor,
...
$\mathcal{V}$ and keep them fixed for model selection
...
($\mu\text{-risk}_{IPW}$) does not bring much improvement. The
$R\text{-risk}$, which includes a model of mean outcome and propensity
scores, leads to the best performance. Interestingly, the
...
nuisance estimates is a form of double dipping which can lead to errors in
...
metrics with close performance to the oracles ones: the
...
standard cross-validation used \emph{\eg}~in machine learning: it involves
...
data is simplified, mainly by centralizing fragmented data from poor
interoperability and the use of NLP techniques. The main aim of these
CDWs is to improve clinical research. However, deriving new evidence from EHRs
...
%
\paragraph{These data repositories are not big enough for large neural networks
...
\begin{appendices}
      ...
      \resizebox{1.4\textwidth}{!}{%
        \begin{tabular}{|l|l|}
          ...
          \begin{tabular}[c]{@{}l@{}}Health Technology Assessment Department:
            ...
            oncology). Evaluation of procedures: \eg biological and imaging
  ...
  studies), half multi-center studies. The tasks and performance were
  ...
  predictive performance (measured both on AUOC, AUPRC and ranking metrics) as
  well as calibration performance; 2) to detail the performance with the number
  ...
  \begin{figure}[htbp]
    ...
    \end{center}
    \caption{BEHRT \citep{li2020behrt} performance for every diagnosis target.
  ...
  \ref{apd:fig:behrt_performances} outlines the performance of BEHRT on every
  diagnosis. There is no clear trend that outlines better performance for higher
  ...
  \begin{figure}[!h]
    ...
    \caption{LOS AUPRC for different featurizers and estimators.
      The performance is averaged over 5 folds. The shaded area represents the
  ...
  Performance is reported with Area Under the Receiver Operating
  ...
  \begin{figure}[!h]
    ...
    \caption{Prognosis ROC AUC averaged over chapters for different featurizers and
      estimators. The performance is averaged over 5 folds. The shaded area
      ...
      Random forest have better performance. Count encoder outperform other
  ...
  \begin{figure}[!h]
    ...
    \caption{Prognosis AUPRC averaged by prevalence over chapters
      for different featurizers and estimators. The performance is averaged
      over 5 folds. The shaded area represents the standard deviation. The
  ...
  \begin{figure}[!h]
    ...
    \caption{LOS task, transfer between hospitals with 2100 codes only, ROC AUC:
      ...
      little bit the performance for logistic regression but decreases the performance of the
  ...
  important role in the performance of the different pipelines. We evaluated this
  ...
  performance. This failure illustrates well the shortcuts on which predictive
  ...
  generalization performance and counterfactual prediction aiming at unbiased
  ...
  $U$-risk has poor performance at bad overlap. Estimating these propensity scores in the is feasible $U$-risk reduces
  ...
  \begin{figure}
    ...
    \begin{subfigure}[b]{0.10\textwidth}
      ~
    \end{subfigure}
    \caption{Metric performance by normalized tau-risk distance to the best
...
\begin{mdframed}[linecolor=Prune,linewidth=1]
  ...
  électroniques, Causalité, Epidémiologie, Santé publique
...
\begin{mdframed}[linecolor=Prune,linewidth=1]
  ...
  \noindent \textbf{Keywords:} Machine learning, Electronic Health Records, Causality, Epidemiology, Public health
  ...
  \begin{multicols}{2}
    ...
    to improve her health? In order to gain a better understanding of the modern
    infrastructures for collecting and analyzing Electronic Health Records
    (EHRs), we summarize semi-structured interviews conducted as part of a
    national case study of the clinical data warehouses (CDWs) of the 32 French
    regional and university hospitals. Acknowledging the difficulty to access
    large sample sizes and computational power to develop generalizable
    ...
    Information Mart for Intensive Care database (MIMIC-IV). EHRs are
    high-dimensional databases. For such settings, the selection of
    hyper-parameters for the causal model is crucial to avoid under- or
    over-learning. In a simulation and three semi-simulated datasets, we show
    that the usual machine learning risk are not adapted to the causal setting
    and that the doubly robust R-risk outperforms other existing causal risks.
  \end{multicols}
\end{mdframed}
